...
func RegisterCaptureResponse(service *Capture, ctx context.Context) error {
	name := "capture_response"
	sample := &msgbus.Event{Workspace: "*", App: "*", Type: events.SCHEDULE_RESPONSE}
	queue := fmt.Sprintf("%s_%s", name, service.app.Configs.MsgBus.QueueName)
	cleanup, err := service.app.MsgBus.Sub(ctx, sample, queue, UseCaptureResponse(service.app))
	if err != nil {
		return err
	}
